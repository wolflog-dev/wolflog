...
// Configuration locale (non écrasée par les mises à jour) : wolflog.json à côté du binaire, puis /etc/wolflog/wolflog.json.
...
builder.Services.Configure<WolflogServerOptions>(builder.Configuration.GetSection(WolflogServerOptions.Section));
...
builder.Services.AddSingleton<Wolflog.Server.Dashboards.DashboardStore>();
builder.Services.AddSingleton<Wolflog.Server.Configuration.ErrorStateStore>();
builder.Services.AddSingleton<Wolflog.Server.Configuration.DeploymentStore>();
builder.Services.AddSingleton<Wolflog.Server.Configuration.SavedSearchStore>();

// Surveillance : alertes, sondes, SLO, santé de Wolflog.
...
// Sources lues directement par Wolflog : fichiers de logs et syslog.
builder.Services.AddSingleton<Wolflog.Server.Sources.LogSourceStore>();
builder.Services.AddSingleton<Wolflog.Server.Sources.SourceHost>();
builder.Services.AddHostedService(sp => sp.GetRequiredService<Wolflog.Server.Sources.SourceHost>());
...
var serverOptions = builder.Configuration.GetSection(WolflogServerOptions.Section).Get<WolflogServerOptions>() ?? new WolflogServerOptions();
