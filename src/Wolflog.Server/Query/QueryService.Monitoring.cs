...
public sealed partial class QueryService
{
    ...
    public IReadOnlyList<ProbeStat> ProbeStats(DateTime from, DateTime to, int buckets, CancellationToken ct)
    {
        ...
        Read($"SELECT probe, count(*), avg(value) FROM {ProbeSource(from, to, "wolflog.probe.up")} WHERE probe IS NOT NULL GROUP BY probe", ct,
        ...
        Read($"""
            ...
            FROM {ProbeSource(from, to, "wolflog.probe.up")} WHERE probe IS NOT NULL GROUP BY 1, 2
            """, ct, r =>
        {
        ...
        Read($"SELECT probe, avg(value), quantile_cont(value, 0.95) FROM {ProbeSource(from, to, "wolflog.probe.duration")} WHERE probe IS NOT NULL GROUP BY probe", ct,
    ...
    {
        (long, long) result = (0, 0);
        Read($"SELECT count(*), count(*) FILTER (WHERE value < 1) FROM {ProbeSource(from, to, "wolflog.probe.up")} WHERE probe = {Sql.Str(probeId)}", ct,
            r => result = (r.GetInt64(0), r.GetInt64(1)));
        return result;
    }
    ...
    public IReadOnlyList<GoodBadPoint> ProbeGoodBadSeries(string probeId, DateTime from, DateTime to, int stepSeconds, CancellationToken ct)
    {
        ...
        Read($"""
            ...
            FROM {ProbeSource(from, to, "wolflog.probe.up")} WHERE probe = {Sql.Str(probeId)} GROUP BY 1
            """, ct, r => map[r.GetInt64(0)] = (r.GetInt64(1), r.GetInt64(2)));
        return Buckets(from, to, stepMs, map);
    }
