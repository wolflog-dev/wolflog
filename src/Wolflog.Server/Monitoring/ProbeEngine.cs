...
/// <summary>Exécute les sondes et enregistre les résultats comme métriques (wolflog.probe.up, wolflog.probe.duration).</summary>
public sealed class ProbeEngine(ProbeStore probes, Ingestor ingestor, IHttpClientFactory http, ILogger<ProbeEngine> log) : BackgroundService
{
    ...
    private async Task<ProbeResult> CheckAsync(Probe p, CancellationToken ct)
    {
        ...
        try
        {
            ...
            request.Headers.UserAgent.ParseAdd("Wolflog-Probe/1.0");
    ...
    private async Task RecordAsync(Probe p, ProbeResult r, CancellationToken ct)
    {
        ...
        var scope = new ScopeMetrics { Scope = new InstrumentationScope { Name = "Wolflog.Probes" } };
        scope.Metrics.Add(Gauge("wolflog.probe.up", "1", r.Ok ? 1 : 0));
        scope.Metrics.Add(Gauge("wolflog.probe.duration", "ms", r.DurationMs));
        if (r.CertificateDays is { } days) scope.Metrics.Add(Gauge("wolflog.probe.certificate_days", "d", days));
        var request = new ExportMetricsServiceRequest
        {
            ResourceMetrics =
            {
                new ResourceMetrics
                {
                    Resource = new Resource { Attributes = { Kv("service.name", string.IsNullOrWhiteSpace(p.Service) ? "wolflog-sondes" : p.Service) } },
                    ScopeMetrics = { scope },
                },
            },
        };
