...
/// ou écoute syslog, et les envoie à un serveur Wolflog distant en OTLP.
/// </summary>
public static class Agent
{
    ...
    public const string Help = """
        Mode agent : lit des fichiers de logs sur cette machine et les envoie à un serveur Wolflog.

          wolflog agent --endpoint https://wolflog:5080 --key <clé> --file "C:\inetpub\logs\LogFiles\W3SVC1\*.log" --format iis --service mon-site
    ...
    public static int Run(string[] args)
    {
        ...
        if (!string.IsNullOrEmpty(config.ApiKey)) http.DefaultRequestHeaders.Add("x-wolflog-key", config.ApiKey);
        var positions = new FilePositions(Path.Combine(AppContext.BaseDirectory, "wolflog-agent-positions.json"));
