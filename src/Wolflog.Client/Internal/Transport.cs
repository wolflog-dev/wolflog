...
/// <summary>
/// Envoi vers le serveur Wolflog : compression gzip, clé API, et mise en tampon disque
/// quand le serveur est injoignable (renvoi automatique ensuite). L'application n'est jamais bloquée.
/// </summary>
internal sealed class WolflogTransport : IDisposable
{
    ...
    public WolflogTransport(WolflogOptions options, string bufferDirectory)
    {
    ...
    private HttpRequestMessage BuildRequest(string signal, byte[] gzipped)
    {
        ...
        if (!string.IsNullOrEmpty(_apiKey)) request.Headers.TryAddWithoutValidation("x-wolflog-key", _apiKey);
        return request;
    }
    ...
    private SendResult Classify(HttpStatusCode status)
    {
        ...
        if (code is 401 or 403)
            Trace.TraceWarning($"Wolflog : accès refusé par le serveur ({code}). Vérifiez Wolflog:ApiKey.");
        MarkUnavailable();
        return SendResult.Retry;
    }
    ...
    private async Task RetryLoop()
    {
        ...
        while (!_stop.IsCancellationRequested)
        {
            ...
            {
                Trace.TraceWarning($"Wolflog : erreur lors du renvoi du tampon : {ex.Message}");
    ...
    /// </summary>
    private sealed class ExportHandler(WolflogTransport transport) : HttpMessageHandler
    {
