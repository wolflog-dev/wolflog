...
/// <summary>Instance d'application qui peut être profilée (paquet Vigil.Client.Profiling), vue récemment.</summary>
...
public sealed class ProfileStore : JsonCollection<ProfileInfo>
{
    ...
    public ProfileStore(IOptions<VigilServerOptions> o, IHostEnvironment env)
    ...
    /// <summary>Identifiant généré par Vigil (lettres et chiffres) : jamais un chemin.</summary>
    ...
    public void Expire()
    {
        foreach (var p in All().Where(p => p.Status is "pending" or "running" && DateTime.UtcNow - p.RequestedAt > TimeSpan.FromSeconds(p.Seconds + 90)))
        {
            var error = p.Status == "pending"
                ? "Aucune instance n'a pris la demande (paquet Vigil.Client.Profiling installé ?)"
