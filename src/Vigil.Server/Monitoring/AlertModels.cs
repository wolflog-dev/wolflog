...
public static class AlertKinds
{
    ...
    /// <summary>Santé de Vigil lui-même (disque, écriture, réception).</summary>
...
public sealed class NotificationSettings : IEntity
{
    ...
    /// <summary>Adresse publique de Vigil, pour les liens dans les notifications (ex. https://vigil.mondomaine.fr).</summary>
...
public sealed class AlertRuleStore(IOptions<VigilServerOptions> o, IHostEnvironment env)
    : JsonCollection<AlertRule>(o.Value.ResolveDataDirectory(env.ContentRootPath), "alert-rules.json");

public sealed class AlertChannelStore(IOptions<VigilServerOptions> o, IHostEnvironment env)
    : JsonCollection<AlertChannel>(o.Value.ResolveDataDirectory(env.ContentRootPath), "alert-channels.json");

public sealed class NotificationSettingsStore(IOptions<VigilServerOptions> o, IHostEnvironment env)
...
public sealed class AlertStateStore(IOptions<VigilServerOptions> o, IHostEnvironment env)
    : JsonCollection<AlertState>(o.Value.ResolveDataDirectory(env.ContentRootPath), "alert-states.json");

public sealed class AlertEventStore(IOptions<VigilServerOptions> o, IHostEnvironment env)
