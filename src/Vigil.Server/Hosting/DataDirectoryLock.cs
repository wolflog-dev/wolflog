...
public sealed class DataDirectoryLock : IHostedService, IDisposable
{
    ...
    public DataDirectoryLock(IOptions<VigilServerOptions> options, IHostEnvironment env, ILogger<DataDirectoryLock> log)
    {
    ...
    private void Acquire()
    {
        ...
        while (true)
        {
            ...
            catch (IOException) when (DateTime.UtcNow < deadline)
            {
                if (!warned)
                {
                    _log.LogWarning("Le dossier de données est utilisé par une autre instance de Vigil, attente…");
                    warned = true;
                }
                Thread.Sleep(500);
            }
            catch (IOException ex)
            {
                throw new InvalidOperationException(
                    $"Le dossier de données est déjà utilisé par une autre instance de Vigil ({_path}). " +
