...
using static Vigil.Server.Api.ApiEndpoints;

namespace Vigil.Server.Api;

/// <summary>Alertes, canaux de notification, sondes, SLO, santé de Vigil, sauvegardes.</summary>
public static class MonitoringEndpoints
{
    ...
    {
        public void MapVigilMonitoring(RouteGroupBuilder api, RouteGroupBuilder editor, RouteGroupBuilder admin)
        {
            // ------------------------------------------------------------ alertes
            ...
            api.MapGet("/health/vigil", (HealthService health) => Results.Ok(health.Check()));
            ...
            admin.MapGet("/admin/backup", async (HttpContext ctx, StorageHost storage, BackupState state) =>
            {
                ...
                ctx.Response.Headers.ContentDisposition = $"attachment; filename=\"vigil-{(includeData ? "complet" : "config")}-{DateTime.Now:yyyyMMdd-HHmm}.zip\"";
            ...
            admin.MapPost("/admin/restore", async (HttpContext ctx, StorageHost storage, Dashboards.DashboardStore dashboards) =>
            {
                ...
                try
                {
                    // Configuration seulement : les données se restaurent serveur arrêté (vigil restore).
                    var result = Backup.Restore(storage.DataDirectory, buffer, includeData: false);
                    dashboards.Reload();
                    return Results.Ok(result);
                }
                catch (InvalidDataException) { return Results.BadRequest(new { error = "Ce fichier n'est pas une sauvegarde Vigil valide." }); }
            }).DisableAntiforgery();
        }
    }
