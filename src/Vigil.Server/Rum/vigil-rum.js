/* Vigil RUM : erreurs JavaScript, chargement des pages, appels réseau et Web Vitals.
   <script src="https://vigil.exemple.fr/vigil-rum.js" defer data-key="vgb_…" data-service="mon-site" data-env="prod"></script> */
(function () {
  'use strict';
  var script = document.currentScript;
  if (!script || window.__vigilRum) return;
  window.__vigilRum = true;
  ...
  var session;
  try {
    session = sessionStorage.getItem('vigil.session');
    if (!session) { session = hex(8); sessionStorage.setItem('vigil.session', session); }
  ...
  XMLHttpRequest.prototype.open = function (method, url) {
    this.__vigil = { method: String(method).toUpperCase(), info: traced(url) };
    return open.apply(this, arguments);
  };
  XMLHttpRequest.prototype.send = function () {
    var v = this.__vigil, xhr = this;
    if (v && v.info) {
      if (v.info.inject) {
