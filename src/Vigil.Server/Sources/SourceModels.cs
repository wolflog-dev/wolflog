...
/// <summary>Source de logs lue par Vigil lui-même (sans bibliothèque dans l'application).</summary>
...
public sealed class LogSourceStore(IOptions<VigilServerOptions> o, IHostEnvironment env)
...
public static class OtlpBuilder
{
        ...
        IReadOnlyCollection<ParsedEntry> entries, string defaultService, string? env, string sourceName)
    {
        ...
        {
            var resource = new Resource { Attributes = { Kv("service.name", group.Key.Service), Kv("vigil.source", sourceName) } };
            ...
            var scopeLogs = new ScopeLogs { Scope = new InstrumentationScope { Name = "Vigil.Sources" } };
            var scopeSpans = new ScopeSpans { Scope = new InstrumentationScope { Name = "Vigil.Sources" } };
