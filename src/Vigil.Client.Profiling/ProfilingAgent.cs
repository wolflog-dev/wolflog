...
{
    /// <summary>
    /// Demande Vigil toutes les 10 secondes s'il faut profiler cette instance ; si oui, profile et envoie le résultat.
    /// Aucun coût tant qu'aucun profil n'est demandé.
    /// </summary>
    internal sealed class ProfilingAgent(VigilOptions options, ServiceIdentity identity, ILogger<ProfilingAgent> log) : BackgroundService
    {
        ...
        protected override async Task ExecuteAsync(CancellationToken stop)
        {
            ...
            if (!string.IsNullOrEmpty(options.ApiKey)) http.DefaultRequestHeaders.Add("x-vigil-key", options.ApiKey);
            ...
            while (!stop.IsCancellationRequested)
            {
                ...
                {
                    log.LogDebug(ex, "Vigil : interrogation du profilage impossible");
                }
        ...
        {
            log.LogInformation("Vigil : profil {Kind} de {Seconds} s demandé", request.Kind, request.Seconds);
            ...
            {
                // Ex. EventPipe désactivé (DOTNET_EnableDiagnostics=0) : l'erreur est affichée dans Vigil.
                error = ex.Message;
            ...
            if (!response.IsSuccessStatusCode) log.LogWarning("Vigil : envoi du profil refusé ({Status})", (int)response.StatusCode);
        }
    }
}
...
{
    /// <summary>Profilage à la demande depuis l'interface de Vigil.</summary>
    public static class VigilProfilingExtensions
    {
        extension(IHostApplicationBuilder builder)
        {
            /// <summary>
            /// Active le profilage à la demande (CPU, allocations) depuis Vigil. À appeler après <c>AddVigil()</c>.
            /// </summary>
            public IHostApplicationBuilder AddVigilProfiling()
            {
                builder.Services.AddVigilProfiling();
                return builder;
            }
        }
        ...
        {
            /// <summary>Active le profilage à la demande depuis Vigil. À appeler après <c>AddVigil()</c>.</summary>
            public IServiceCollection AddVigilProfiling()
            {
