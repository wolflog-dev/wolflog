...
// Espace de noms de l'hôte : AddVigil() est disponible sans "using" supplémentaire.
namespace Microsoft.Extensions.Hosting;

/// <summary>Intégration de Vigil dans une application .NET.</summary>
public static class VigilExtensions
{
    ...
    {
        /// <summary>
        /// Envoie les logs, traces, métriques et crashs de l'application vers Vigil.
        /// La configuration est lue dans la section "Vigil" (Endpoint, ApiKey…).
        /// </summary>
        public TBuilder AddVigil(Action<VigilOptions>? configure = null)
        {
            builder.Services.AddVigil(builder.Configuration, builder.Environment, configure);
            return builder;
        }
    }
    ...
    {
        /// <summary>Variante pour IHostBuilder / Startup : services.AddVigil(configuration, environment).</summary>
        public IServiceCollection AddVigil(IConfiguration configuration, IHostEnvironment? environment = null, Action<VigilOptions>? configure = null)
        {
            if (services.Any(d => d.ServiceType == typeof(VigilTransport))) return services; // déjà configuré

            var options = new VigilOptions();
            configuration.GetSection(VigilOptions.Section).Bind(options);
            configure?.Invoke(options);

            if (!options.Enabled) return services;
            if (string.IsNullOrWhiteSpace(options.Endpoint))
            {
                Trace.TraceWarning("Vigil : aucun Endpoint configuré (section \"Vigil\"), rien ne sera envoyé.");
                return services;
            }
            ...
            var bufferDirectory = options.BufferDirectory
                ?? Path.Combine(Path.GetTempPath(), "vigil", string.Concat(identity.Name.Select(c => char.IsLetterOrDigit(c) || c is '-' or '_' or '.' ? c : '_')));

            var transport = new VigilTransport(options, bufferDirectory);
            ...
            services.AddHostedService<VigilLifecycleService>();
