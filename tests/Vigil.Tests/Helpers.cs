...
{
    public string Path { get; } = System.IO.Path.Combine(System.IO.Path.GetTempPath(), "vigil-tests", Guid.NewGuid().ToString("N"));
...
internal sealed class TestEnv(string root) : Microsoft.Extensions.Hosting.IHostEnvironment
{
    ...
    public string ApplicationName { get; set; } = "Vigil.Tests";
...
{
    public static StorageHost CreateStorage(string dataDir, Action<VigilServerOptions>? configure = null)
    {
        var options = new VigilServerOptions { DataDirectory = dataDir };
