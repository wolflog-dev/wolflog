...
public class FeatureTests(VigilServerFixture server) : IClassFixture<VigilServerFixture>
{
    ...
    /// <summary>Application ASP.NET Core réelle (TestServer) instrumentée par Vigil.</summary>
    private async Task<WebApplication> StartApp(string service, string env, string buffer, HttpBodyCapture bodies)
    {
        ...
        builder.AddVigil(o =>
        {
            ...
            o.IgnoredPaths.AddRange(["/v1", "/api"]); // serveur Vigil de test dans le même processus
