...
public class EndToEndTests(WolflogServerFixture server) : IClassFixture<WolflogServerFixture>
{
    ...
    public async Task Otlp_json_is_accepted_with_hex_ids()
    {
        ...
        request.Headers.Add("Authorization", "Bearer " + WolflogServerFixture.ApiKey);
    ...
    public async Task Client_library_sends_logs_traces_and_metrics()
    {
        ...
        using (var app = BuildApp(service, buffer.Path))
        {
            ...
            using var source = new ActivitySource("Wolflog.Tests.Orders");
            var tracer = app.Services.GetRequiredService<OpenTelemetry.Trace.TracerProvider>();
            Assert.NotNull(tracer);
            using (var activity = new ActivitySource("Wolflog.Tests").StartActivity("Commande"))
            {
            ...
            var counter = new System.Diagnostics.Metrics.Meter("Wolflog.Tests").CreateCounter<long>("tests.orders");
            counter.Add(5);
    ...
    public async Task Serilog_events_are_forwarded()
    {
        ...
        using (var app = BuildApp(service, buffer.Path, configure: b =>
                   b.Services.AddSerilog((sp, lc) => lc.MinimumLevel.Debug().WriteTo.Wolflog(sp))))
        {
            await app.StartAsync();
            var log = app.Services.GetRequiredService<ILogger<EndToEndTests>>();
            log.LogWarning("Via MEL {Value}", 1);
            Serilog.Log.Logger = new LoggerConfiguration().WriteTo.Wolflog().CreateLogger();
    ...
    public async Task Crash_reports_and_abnormal_terminations_appear_as_crashes()
    {
        ...
        var transport = app.Services.GetRequiredService<WolflogTransport>();
        transport.Outbox.Store("logs", WolflogTransport.Gzip(report.ToByteArray()));
        await transport.DrainOutboxAsync(default);
        ...
        Assert.Contains("Wolflog.AbnormalTermination", types);
    ...
    public async Task Live_tail_streams_new_logs()
    {
        ...
        var rows = Wolflog.Server.Ingestion.OtlpConverter.ConvertLogs(Otlp.Logs("tail-svc", 5));
