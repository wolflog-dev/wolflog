...
public class WorkflowTests(WolflogServerFixture server) : IClassFixture<WolflogServerFixture>
{
    ...
    private async Task<HttpResponseMessage> SendLogs(ExportLogsServiceRequest request, string key)
    {
        ...
        message.Headers.Add("x-wolflog-key", key);
    ...
    public async Task Resolved_errors_come_back_as_regressed()
    {
        ...
        (await SendLogs(Exception(service, "1.0.0", type, t0), WolflogServerFixture.ApiKey)).EnsureSuccessStatusCode();
        ...
        (await SendLogs(Exception(service, "1.0.1", type, DateTime.UtcNow.AddSeconds(1)), WolflogServerFixture.ApiKey)).EnsureSuccessStatusCode();
    ...
    public async Task Logs_can_be_exported_as_csv_and_json()
    {
        ...
        (await SendLogs(request, WolflogServerFixture.ApiKey)).EnsureSuccessStatusCode();
        var ui = await server.LoggedInClient();
