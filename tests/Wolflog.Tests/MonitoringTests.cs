...
/// <summary>Alertes et notifications, sondes, SLO, santé de Wolflog, sauvegardes.</summary>
public class MonitoringTests(WolflogServerFixture server) : IClassFixture<WolflogServerFixture>
{
    ...
    {
        var ui = await server.LoggedInClient();
        var health = await Get(ui, "/api/health/wolflog");
        ...
        // La sauvegarde est notée dans la santé de Wolflog.
        health = await Get(ui, "/api/health/wolflog");
