// Banc d'essai : envoie des logs OTLP en parallèle à un serveur Vigil, puis mesure quelques requêtes.
//   dotnet run -c Release --project tests/Vigil.Bench -- --url http://localhost:5080 --key <clé> --password <mdp>
...
await Task.WhenAll(Enumerable.Range(0, opts.Concurrency).Select(async worker =>
{
    ...
    while (DateTime.UtcNow < stop)
    {
        ...
        if (opts.Key != null) request.Headers.Add("x-vigil-key", opts.Key);
